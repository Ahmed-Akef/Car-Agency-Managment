...
namespace Car_Agency_Management.Data
{
    public class DB
    {
        ...
        public DB()
        {
            ...
            EnsureMaintenanceAndImportSchema();
        }
        ...
        private void EnsureMaintenanceAndImportSchema()
        {
            try
            {
                if (_connection.State == ConnectionState.Closed) _connection.Open();
                ...
                string checkQuery = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'SpareParts'";
                SqlCommand checkCmd = new SqlCommand(checkQuery, _connection);
                int count = (int)checkCmd.ExecuteScalar();

                if (count == 0)
                {
                    // Create Tables for Maintenance and Imports
                    string createTablesQuery = @"
                        ...
                        CREATE TABLE MaintenanceHistory (
                            ...
                            PartsUsed NVARCHAR(MAX),
                            TotalCost DECIMAL(10,2),
                            TechnicianID INT,
                            CompletionDate DATETIME DEFAULT GETDATE(),
                            FOREIGN KEY (ReportID) REFERENCES MaintenanceReports(ReportID)
                        );
                        ...
                        -- Initial Data
                        INSERT INTO SpareParts (PartName, Price, StockQuantity, Category, Description)
                        VALUES 
                            ('Oil Filter', 550.00, 50, 'Engine', 'High-quality oil filter for most car models'),
                            ('Brake Pads', 900.00, 30, 'Brakes', 'Premium brake pads with extended warranty'),
                            ('Air Filter', 400.00, 40, 'Engine', 'Air filter for optimal engine performance'),
                            ('Spark Plugs', 100.00, 100, 'Engine', 'Set of 4 spark plugs'),
                            ('Battery', 1100.00, 20, 'Electrical', '12V car battery with 2-year warranty'),
                            ('Wiper Blades', 250.00, 60, 'Accessories', 'Pair of all-weather wiper blades'),
                            ('Radiator', 1700.00, 15, 'Cooling', 'Complete radiator assembly');

                        INSERT INTO cars (id, make, model, year, price, image) VALUES
                        (1, 'Xiaomi', 'SU7 ultra', 2025, 3500000, 'https://legion-images.hatla2ee.com/original_image/fdd163f6-178a-412d-a997-7da5c3e647f8/large.jpg'),
                        (2, 'Mercedes', 'amg gt 63', 2025, 1200000, ''),
                        (3, 'GMC', 'hummer EV', 2025, 1100000, ''),
                        (4, 'Xiaomi', 'yu7', 2026, 1800000, ''),
                        (5, 'Cadillac', 'Escalade IQ', 2026, 2500000, ''),
                        (6, 'Deepal', 'G318', 2025, 3500000, ''),
                        (7, 'BMW', 'M5 CS', 2025, 950000, ''),
                        (8, 'BYD', 'leopard5', 2025, 2200000, '');
                    ";
                    
                    // Create Stored Procedures separately to avoid batch errors
                    string sp1 = @"
                        CREATE PROCEDURE sp_SubmitMaintenanceReport
                            @UserID INT,
                            @CarModel NVARCHAR(100),
                        ...
                        AS
                        BEGIN
                            INSERT INTO MaintenanceReports (UserID, CarModel, IssueType, IssueDescription, Status)
                            VALUES (@UserID, @CarModel, @IssueType, @IssueDescription, 'Pending');
                            
                            SELECT SCOPE_IDENTITY() AS NewReportID;
                        END";

                     string sp2 = @"
                        CREATE PROCEDURE sp_SearchSpareParts
                            @SearchTerm NVARCHAR(100)
                        AS
                        BEGIN
                            SELECT PartID, PartName, Price, StockQuantity, Category, Description
                            FROM SpareParts
                            WHERE (PartName LIKE '%' + @SearchTerm + '%' OR Description LIKE '%' + @SearchTerm + '%')
                                AND StockQuantity > 0
                            ORDER BY PartName;
                        END";

                    using (SqlCommand cmd = new SqlCommand(createTablesQuery, _connection))
                    {
                        cmd.ExecuteNonQuery();
                        Console.WriteLine("AUTO-REPAIR: Created Maintenance and Import tables.");
                    }

                    // Execute SPs
                     using (SqlCommand cmd = new SqlCommand(sp1, _connection))
                    {
                         try { cmd.ExecuteNonQuery(); } catch { /* Ignore if exists */ }
                    }
                     using (SqlCommand cmd = new SqlCommand(sp2, _connection))
                    {
                         try { cmd.ExecuteNonQuery(); } catch { /* Ignore if exists */ }
                    }
                }
        ...
        public List<MaintenanceReportModel> GetAllMaintenanceReports()
        {
            List<MaintenanceReportModel> reports = new List<MaintenanceReportModel>();
            string query = @"
                SELECT 
                    r.ReportID, 
                    r.UserID, 
                    ...
                    r.Status, 
                    r.ReportDate
                FROM MaintenanceReports r
                LEFT JOIN CUSTOMER c ON r.UserID = c.CUSTOMER_ID
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    reports.Add(new MaintenanceReportModel
                    {
                        ReportID = Convert.ToInt32(reader["ReportID"]),
                        UserID = reader["UserID"] != DBNull.Value ? Convert.ToInt32(reader["UserID"]) : 0,
                        CustomerName = reader["CustomerName"] != DBNull.Value ? reader["CustomerName"].ToString() : "Unknown",
                        ...
                        ReportDate = reader["ReportDate"] != DBNull.Value ? Convert.ToDateTime(reader["ReportDate"]) : DateTime.MinValue
                    });
                }
                reader.Close();
            }
            ...
            {
                Console.WriteLine($"Error getting maintenance reports: {ex.Message}");
            }
            finally
            {
        ...
        public List<MaintenanceReportModel> GetUserMaintenanceReports(int userId)
        {
            List<MaintenanceReportModel> reports = new List<MaintenanceReportModel>();
            string query = @"
                SELECT 
            ...
            {
                Console.WriteLine($"Error getting user maintenance reports: {ex.Message}");
            }
            finally
            {
                if (_connection.State == ConnectionState.Open) _connection.Close();
        ...
        public List<SparePart> GetSpareParts(string searchTerm = "")
        {
            ...
            try
            {
                ...
                {
                    query = "SELECT PartID, PartName, Price, StockQuantity, Category, Description FROM SpareParts WHERE StockQuantity > 0 ORDER BY PartName";
                    cmd = new SqlCommand(query, _connection);
                }
                else
                {
                    // Use Stored Procedure if preferred, or direct query
                    query = "sp_SearchSpareParts";
                    cmd = new SqlCommand(query, _connection);
            ...
            {
                Console.WriteLine($"Error getting spare parts: {ex.Message}");
            }
            finally
            {
                if (_connection.State == ConnectionState.Open) _connection.Close();
        ...
        public (bool Success, string Message) SubmitMaintenanceReport(int userId, string carModel, string issueType, string description)
        {
            try
            {
                _connection.Open();
                ...
                string checkUser = "SELECT COUNT(*) FROM CUSTOMER WHERE CUSTOMER_ID = @ID";
                SqlCommand checkCmd = new SqlCommand(checkUser, _connection);
                checkCmd.Parameters.AddWithValue("@ID", userId);
                int count = (int)checkCmd.ExecuteScalar();
                
                if (count == 0)
                {
                    // If hardcoded user 1 doesn't exist, try to find ANY user
                    string findUser = "SELECT TOP 1 CUSTOMER_ID FROM CUSTOMER";
                    SqlCommand findCmd = new SqlCommand(findUser, _connection);
                    object result = findCmd.ExecuteScalar();
                    
                    if (result != null)
                    {
                        userId = Convert.ToInt32(result); // Use an existing user
                    }
                    else
                    {
                         // No users exist at all? Create one or fail
                         return (false, "No users found in database. Please Register a user first.");
                    }
                }

                SqlCommand cmd = new SqlCommand("sp_SubmitMaintenanceReport", _connection);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@UserID", userId);
                cmd.Parameters.AddWithValue("@CarModel", carModel);
                cmd.Parameters.AddWithValue("@IssueType", issueType);
                cmd.Parameters.AddWithValue("@IssueDescription", description);
                
                // If the SP doesn't exist (maybe created in different batch), fallback to inline insert
                try 
                {
                     cmd.ExecuteNonQuery();
                }
                catch (SqlException ex) when (ex.Message.Contains("Could not find stored procedure"))
                {
                     ...
                     cmd.CommandText = "INSERT INTO MaintenanceReports (UserID, CarModel, IssueType, IssueDescription, Status) VALUES (@UserID, @CarModel, @IssueType, @IssueDescription, 'Pending')";
                     cmd.ExecuteNonQuery();
                }
        ...
        // ============================================
        // IMPORTED CARS OPERATIONS
        // ============================================

        public List<ImportedCar> GetAllImportedCars()
        {
            List<ImportedCar> cars = new List<ImportedCar>();
            try
            {
                _connection.Open();
            ...
            {
                 Console.WriteLine($"Error getting imported cars: {ex.Message}");
            }
            finally
            {
                 if (_connection.State == ConnectionState.Open) _connection.Close();
        ...
        public bool AddImportRequest(ImportRequest request)
        {
            try
            {
                _connection.Open();
                // We use simple ID generation or let auto-id if we changed DB schema, but user schema has BIGINT PRIMARY KEY
                // We will generate a random ID or timestamp if not IDENTITY. 
                // User schema says: id BIGINT PRIMARY KEY
                string query = @"INSERT INTO import_requests (id, car_id, car_name, car_year, customer_name, email, phone, message, request_date)
                               VALUES (@Id, @CarId, @CarName, @CarYear, @CustName, @Email, @Phone, @Message, GETDATE())";
                
                SqlCommand cmd = new SqlCommand(query, _connection);
                cmd.Parameters.AddWithValue("@Id", DateTime.Now.Ticks); // Simple Unique ID
                cmd.Parameters.AddWithValue("@CarId", request.CarId);
                cmd.Parameters.AddWithValue("@CarName", request.CarName);
                cmd.Parameters.AddWithValue("@CarYear", request.CarYear);
                cmd.Parameters.AddWithValue("@CustName", request.CustomerName);
            ...
            {
                 Console.WriteLine($"Error adding import request: {ex.Message}");
                 return false;
            }
            finally
            {
        ...
        public List<ImportRequest> GetImportRequests()
        {
            ...
            {
                _connection.Open();
                string query = "SELECT * FROM import_requests ORDER BY request_date DESC";
                SqlCommand cmd = new SqlCommand(query, _connection);
                SqlDataReader reader = cmd.ExecuteReader();
                ...
                while (reader.Read())
                {
                    requests.Add(new ImportRequest
                    {
                        ...
                        CarId = reader["car_id"] != DBNull.Value ? Convert.ToInt32(reader["car_id"]) : 0,
                        CarName = reader["car_name"].ToString(),
                        CarYear = reader["car_year"] != DBNull.Value ? Convert.ToInt32(reader["car_year"]) : 0,
            ...
            {
                 Console.WriteLine($"Error getting import requests: {ex.Message}");
            }
            finally
            {
                 if (_connection.State == ConnectionState.Open) _connection.Close();
        ...
        public bool AddSparePart(string partName, decimal price, int stockQuantity, string category, string description)
        {
            try
            {
                _connection.Open();
                string query = "INSERT INTO SpareParts (PartName, Price, StockQuantity, Category, Description, CreatedDate, LastUpdated) VALUES (@PartName, @Price, @StockQuantity, @Category, @Description, GETDATE(), GETDATE())";
                SqlCommand cmd = new SqlCommand(query, _connection);
